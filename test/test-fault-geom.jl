...
@testitem "fault endpoint: Strike/Length move the trace to the geodesic endpoint" tags=[:gui] setup=[GmtvtkTest] begin
	IG = InteractiveGMT; GMT = IG.GMT
	e = iview()
	try
		_register_faultgeom_test()                         # geog path solves the endpoint in Julia (geod)
		IG._pump_once()

		# Inject a 2-vertex fault starting at (lon1,lat1).
		lon1, lat1 = -10.0, 38.0
		n = ccall(_test_fn(:gmtvtk_fault_add_test), Cint,
		...
		nv = ccall(_test_fn(:gmtvtk_fault_apply_test), Cint,
		...
		nc = ccall(_test_fn(:gmtvtk_fault_apply_test), Cint,
