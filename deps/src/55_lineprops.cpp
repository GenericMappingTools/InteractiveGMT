...
static void nestShowInfo(Scene *s, vtkActor *a) {
	...
	showInfoText(s->win, "Info — Nesting info", t);
}
...
// Does this window hold a REAL grid to sample — not the fake 2x2 all-zero placeholder every
// vector-only empty-launcher promote uses to get framed axes without data (drop.jl's zblank=zeros(2,2)
// convention: vector overlays, Background region, the Coastlines/basemap scaffold)? `!gridZ.empty()`
// alone is true for that placeholder too (still a non-empty 4-element vector), which is why a genuine
// grid's node count is the signal — no real grid is ever exactly 2x2. ONE test, used by every menu
// entry that samples the grid ("Extract profile…" on a drawn line AND on the Ctrl-drag track), so the
// gate can never drift between them.
static bool sceneHasRealGrid(Scene *s) {
	return s && !s->gridZ.empty() && s->gnx > 2 && s->gny > 2;
}
	...
	if (!isSlip && lineMeasurable(s, lr)) {
		if (lineClosedRing(s, lr)) {
			...
			if (!isNestRect) {
				...
				// promote uses — the shared gate, same one "Extract profile…" uses.
				const bool hasRealGrid = sceneHasRealGrid(s);
			...
			// grid (sceneHasRealGrid, the shared gate).
			if (sceneHasRealGrid(s))
				m.addAction("Extract profile…", [s, lr]() { lineExtractProfile(s, lr); });
		}
	}
	m.addSeparator();
	if (lr.kind == LK_Profile) {
		// The Ctrl+drag track is a line like any other, so it carries "Extract profile…" too — the SAME
		// action, through the SAME lineExtractProfile, so it re-samples with grdtrack at full grid
		// resolution (the live drag samples 300 points) on the DISPLAYED grid and opens the X,Y plot.
		if (sceneHasRealGrid(s))
			m.addAction("Extract profile…", [s, lr]() { lineExtractProfile(s, lr); });
