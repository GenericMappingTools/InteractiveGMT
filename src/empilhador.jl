...
#
# `empilhador` does the work and reports progress through the `progress` keyword; the dialog side of
# the tool (Tools > Empilhador) is the second half of this file, and calls straight into it.

using GMT: Dates, GMTuserdir, @sprintf		# not exported by GMT, so named one by one
...
# =================================================================================================
# ================================== the dialog side of the tool ==================================
# =================================================================================================
# Tools > "Empilhador". The C++ dialog (EmpilhadorDialog, 70_window.cpp) loads deps/ui/empilhador.ui
# at runtime and drives everything through the two callbacks below.
#
# NONE of the stacking lives here: the engine is `empilhador` above, so the callback and a plain
# `empilhador(...)` call from the console run the SAME code — no GUI-only fork of the maths.
# This part only turns the dialog's "key=value" block into that function's keywords.
...
when "Use config file" is ticked. The FILE is parsed by `_emp_sniff_config` (above) — the one and
only reader of that format — never re-parsed here.
"""
function _emp_config_str()
	L2 = EmpL2()
	_emp_sniff_config(l2config_file(true), L2)		# create a default one if absent
...
# JIT for the whole `empilhador` chain. Pure compute on throw-away data — nothing here touches the
# scene, so no `precompile`-only entries are needed (see warmup.jl's body rule).
function _emp_warm()
	_emp_time_from_name("A2012024021000.L2_LAC_SST4");	yield()
	_emp_flagbits(L2_DEFAULT_FLAGS)
	_emp_akima([1.0, 5, 9, 13], [0.0, 2, 1, 3], collect(1.0:13.0));		yield()
	L2 = EmpL2(active = true, inc = 0.5, ncells = 1, region = (0.0, 4.0, 0.0, 4.0), got_R = true)
	x = repeat(collect(0.0:0.5:4.0), 9);	y = repeat(collect(0.0:0.5:4.0), inner = 9)
	_emp_smart_grid(x, y, x .+ y, L2, 0.0, 4.0, 0.0, 4.0, 9, 9);		yield()
